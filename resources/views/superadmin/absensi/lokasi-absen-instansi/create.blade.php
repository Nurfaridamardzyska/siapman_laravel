    ...
    <form action="{{ route('superadmin.absensi.lokasi-absen-instansi.store') }}" method="POST">
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700">Nama Unit Kerja <span class="text-gray-400 text-xs">(opsional)</span></label>
                    <input type="text" name="unit_name" value="{{ old('unit_name') }}"
                           placeholder="Contoh: Kantor Dinas Pendidikan Lamongan"
                           class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
                    @error('unit_name')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-400">Diisi jika lokasi ini dikhususkan untuk unit kerja atau instansi tertentu.</p>
                </div>

                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700">ID Unit <span class="text-gray-400 text-xs">(opsional)</span></label>
                    <input type="number" name="unit_id" value="{{ old('unit_id') }}"
